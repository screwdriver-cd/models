 ...
 * @return {Promise}
 */
function updateSCM(config) {
    const repoInfo = githubHelper.getInfo(config.scmUrl);
...
class BuildModel extends BaseModel {
    ...
    start(config) {
        ...
        return this.pipeline
            ...
            .then(pipeline =>
                ...
                // get the admin for notification
                .then(() => pipeline.admin)
                // update github
                .then(admin =>
                    updateSCM({
    ...
    stop() {
        ...
        return this.pipeline
            // stop the build
            .then(pipeline =>
                nodeify.withContext(this[executor], 'stop', [{ buildId: this.id }])
                // get the admin for notification
                .then(() => pipeline.admin)
                // update github
                .then(admin =>
            ...
            .then(() => {
                ...
                return this.pipeline
                    .then(pipeline =>
                        pipeline.admin
                        // update github
                            .then(admin => updateSCM({
                                user: admin,
                                scmUrl: pipeline.scmUrl,
                                sha: this.sha,
                                state: (this.status === 'SUCCESS') ? 'success' : 'failure'
                            }))
                            .then(() => this)
                    );
            }
