 ...
 */
function getToken(this, job, pipeline) {
    ...
    const token = this[tokenGen](this.id, tokenGenConfig,
...
class Job extends BaseModel {
     ...
     * @param  {String}     config.queueUri     URI of the queue service
     * See model schema
     * @constructor
     */
    constructor(config) {
            ...
            .then(newJob => pipelineFactory.get(newJob.pipelineId)
                .then(pipeline => {
                    this[executor].startPeriodic({
                        ...
                        token: getToken.call(this, newJob, pipeline),
                        apiUri: this[apiUri],
                        isUpdate: true
                    });
                })
    ...
    remove() {
        const removeBuilds = (() =>
            ...
            Promise.all([this.getBuilds({ paginate: { count: MAX_COUNT } }), this.pipeline, this.job])
                .then(([builds, pipeline, job]) => {

                    const token = getToken.call(this, job, pipeline);
