...
class BuildModel extends BaseModel {
    ...
    /**
     * Lazy load a stageBuild model for the build
     * @property stageBuild
     * @return {Promise}
     */
    async getStageBuild() {
        ...
        const nodes = pipeline.workflowGraph.nodes;
        const node = nodes.find(n => n.name === job.name);
        const stageName = node ? node.stageName : undefined;

        if (stageName) {
            const stageFactory = StageFactory.getInstance();
            const stageBuildFactory = StageBuildFactory.getInstance();

            const stage = await stageFactory.get({ pipelineId: pipeline.id, name: stageName });

            if (!stage || stage.length === 0) {
                throw new Error('Stage does not exist');
            }

            const stageBuild = await stageBuildFactory.get({ params: { eventId: this.eventId, stageId: stage.id } });

            if (!stageBuild) {
                throw new Error('StageBuild does not exist');
            }
