...
const resolveVersionedModel = require('./versionResolver');

const EXACT_VERSION_REGEX = schema.config.regex.EXACT_VERSION;
const VERSION_REGEX = schema.config.regex.VERSION;
const TEMPLATE_NAME_REGEX = schema.config.regex.FULL_TEMPLATE_NAME_WITH_NAMESPACE;
const TEMPLATE_METADATA_FIELDS = ['pipelineId', 'namespace', 'name', 'maintainer', 'latestVersion'];
let instance;

/**
 * Add pipeline template metadata to a template version
 * @param  {PipelineTemplateVersionModel} templateVersion Pipeline template version
 * @param  {Object}                       templateMeta    Pipeline template metadata
 * @return {PipelineTemplateVersionModel}                 Pipeline template version with metadata
 */
function addTemplateMetadata(templateVersion, templateMeta) {
    TEMPLATE_METADATA_FIELDS.forEach(fieldName => {
        templateVersion[fieldName] = templateMeta[fieldName];
    });
...
class PipelineTemplateVersionFactory extends BaseFactory {
    ...
    /**
     * Get a pipeline template by its full name
     * @method getTemplate
     * @param  {String}     fullTemplateName                 Full pipeline template name
     * @param  {Factory}    pipelineTemplateFactory          PipelineTemplateFactory
     * @param  {Factory}    pipelineTemplateTagFactory       PipelineTemplateTagFactory
     * @return {Promise}                                     PipelineTemplateVersionModel
     */
    async getTemplate(fullTemplateName, pipelineTemplateFactory, pipelineTemplateTagFactory) {
        const [, namespace, name, versionOrTag] = TEMPLATE_NAME_REGEX.exec(fullTemplateName);
        const pipelineTemplateMeta = await pipelineTemplateFactory.get({ namespace, name });

        if (!pipelineTemplateMeta) {
            return null;
        }

        const pipelineTemplateVersion = await resolveVersionedModel({
            versionOrTag,
            getExactVersion: version =>
                this.get({ templateId: pipelineTemplateMeta.id, version }, pipelineTemplateFactory),
            getTag: tag => pipelineTemplateTagFactory.get({ namespace, name, tag }),
            getLatestVersion: () => null,
            listVersions: () => this.list({ params: { templateId: pipelineTemplateMeta.id } }, pipelineTemplateFactory)
        });
        ...
        return addTemplateMetadata(pipelineTemplateVersion, pipelineTemplateMeta);
    }
