 ...
 * @param  {Boolean}            multiBuildClusterEnabled        Is multiBuildClusterEnabled in config
 * @return {String}             Build cluster name
 */
async function getBuildClusterName({ annotations, pipeline, isPipelineUpdate = false, multiBuildClusterEnabled, provider }) {
    if (provider) {
...
function getExecutorName(annotations, pipeline, provider) {
    const executorAnnotation = 'screwdriver.cd/executor';
    const pipelineAnnotations = hoek.reach(pipeline, 'annotations', { default: {} });

    const executorFromAnnotations = annotations?.[executorAnnotation] ?? '';
    const executorFromPipelineAnnotations = pipelineAnnotations?.[executorAnnotation] ?? '';
