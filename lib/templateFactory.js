        ...
        return this.list(templateListConfig).then(templates => {
            ...
            return Promise.all([
                ...
                templates.map(t => {
                    ...
                    // The number of pipelines that use this template,
                    // NOT the number of template uses in pipelines
                    const pipelineCount = pCount.filter(p => p.templateId === t.id).length;
    ...
    async getPipelineUsage(fullTemplateName) {
        ...
        return Promise.all(
            pipelineData.map(async p => {
                const eventConfig = {
                    params: { id: p.lastEventId }
                };
                const possibleEvent = await eventFactory.list(eventConfig);
                const event = possibleEvent.length > 0 ? possibleEvent[0].createTime : null;
                return {
                    id: p.id,
                    name: p.name,
                    scmRepo: p.scmRepo,
