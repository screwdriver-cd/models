...
class BuildModel extends BaseModel {
    ...
    updateCommitStatus(pipeline) {
        return Promise.all([
            ...
            return this.scm.updateCommitStatus(config).catch((err) => {
                const shaLimitErr = {
                    statusCode: 500,
                ...
                // TODO: This comparison is dependent on the Github API version; in the future, do not update commit status if build triggered by ~commit
                if (hoek.deepEqual(err, shaLimitErr)) {
                    return null;
                }
