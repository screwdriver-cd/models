...
const { getQueries, PIPELINE_TYPE_BUILD_EVENTS_QUERY } = require('./rawQueries');
let instance;
...
class EventFactory extends BaseFactory {
            ...
            pipelineFactory
                ...
                .then(p => {
                    ...
                    if (parentEventId && !prRef) {
                        // Sync with configPipelineSha for
                        //   - child pipelines restart event
    ...
    async getPipelineTypeBuildEvents(pipelineId) {
        const eventsQueryConfig = {
            queries: getQueries(this.datastore.prefix, PIPELINE_TYPE_BUILD_EVENTS_QUERY),
            readOnly: true,
            replacements: {
                pipelineId
            },
        ...
        return this.query(eventsQueryConfig).then(events => {
            const result = events[0].map(event => {
                return this.createClass(event);
            });
