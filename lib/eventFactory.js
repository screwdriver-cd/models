...
async function getJobParameters(config) {
    ...
    jobs.forEach(job => {
        ...
        if (eventConfig.prNum) {
            ...
            const prJobPrefix = `PR-${eventConfig.prNum}:`;

            if (job.name.startsWith(prJobPrefix) && jobParameters) {
                const baseJobName = job.name.replace(prJobPrefix, '');
        ...
        } else if ((job.prParentJobId === null || job.prParentJobId === undefined) && jobParameters) {
