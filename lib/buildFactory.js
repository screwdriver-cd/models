        ...
        return factory.get(jobId).then(job => {
            ...
            return Promise.all([job.pipeline, getCommitInfo({ job, scm: this.scm, modelConfig })]).then(
                async ([pipeline, data]) => {
                    ...
                    let buildClusterName = await helper.getBuildClusterName({
                        annotations,
                        pipeline,
                    ...
                    let bookendKeyName = await helper.getBookendKeyName({
                        buildClusterName,
                        annotations,
                        pipeline,
                        provider
                    ...
                    const [setup, teardown] = await Promise.all([
                        this.bookend.getSetupCommands(bookendConfig, bookendKeyName),
                        this.bookend.getTeardownCommands(bookendConfig, bookendKeyName)
                    ]);
