...
class PipelineModel extends BaseModel {
    ...
    */
    ...
    async getMetrics(config = { startTime: null, endTime: null }) {
        ...
        const getAllEvents = async (opts, eventArray, date, index) => {
            ...
            let currentDate = date || formatDate(events[0].createTime);
            let currentIndex = index || 0;

            // Create an array where each element is an array of builds of the same date
            for (let i = 0; i < length; i += 1) {
                const currentEvent = events[i];
                const eventDate = formatDate(currentEvent.createTime);

                if (!dayjs(eventDate).isSame(dayjs(currentDate))) {
                    currentIndex += 1;
                    currentDate = eventDate;
                    eventArray.push([]);
                }

                eventArray[currentIndex].push(currentEvent);
            }
            ...
            return getAllEvents(opts, eventArray, currentDate, currentIndex);
        };

        const allEvents = await getAllEvents(options);

        const aggregatedMetrics = [];

        allEvents.forEach((arr) => {
            const metrics = arr.map(e => eventMetrics(e, config.aggregate).duration);
