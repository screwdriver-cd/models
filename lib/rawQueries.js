...
const Queries = {
    ...
    getPipelineTypeBuildEventsQuery: (tablePrefix = '') => `
        SELECT *
        FROM (
            SELECT "id", "parentEventId", "groupEventId", "causeMessage", "createTime", "creator",
                    "pipelineId", "sha", "configPipelineSha", "startFrom", "type", "pr", "prNum", "baseBranch", "meta"
            FROM "${tablePrefix}events"
            WHERE "pipelineId" = :pipelineId
                AND "type" = 'pipeline'
        ) AS filtered_results
        WHERE "meta" LIKE '%"build":%'
        ORDER BY "createTime" DESC`,

    getPipelineTypeBuildEventsQueryMySQL: (tablePrefix = '') => `
        SELECT *
        FROM (
            SELECT id, parentEventId, groupEventId, causeMessage, createTime, creator,
                   pipelineId, sha, configPipelineSha, startFrom, type, pr, prNum, baseBranch, meta
...
const QUERY_MAPPINGS = {
    ...
    PIPELINE_TYPE_BUILD_EVENTS_QUERY: Symbol('get pipeline build event')
};
...
const getQueries = (tablePrefix, querylabel) => {
    switch (querylabel) {
        ...
        case QUERY_MAPPINGS.PIPELINE_TYPE_BUILD_EVENTS_QUERY:
            return [
                { dbType: 'postgres', query: Queries.getPipelineTypeBuildEventsQuery(tablePrefix) },
                { dbType: 'sqlite', query: Queries.getPipelineTypeBuildEventsQuery(tablePrefix) },
                { dbType: 'mysql', query: Queries.getPipelineTypeBuildEventsQueryMySQL(tablePrefix) }
            ];
        default:
...
module.exports = {
    ...
    PIPELINE_TYPE_BUILD_EVENTS_QUERY: QUERY_MAPPINGS.PIPELINE_TYPE_BUILD_EVENTS_QUERY
};
