...
class BuildFactory extends BaseFactory {
     ...
     * @param  {String}     config.scmContext    The scm conext to which user belongs
    ...
    create(config) {
        ...
        const displayName = this.scm.getDisplayName(config)
            ? `${this.scm.getDisplayName(config)}:${config.username}`
            : config.username;
