...
function unsealSecret(secret, pw) {
    return iron.unseal(secret.value, pw, iron.defaults).then(unsealed => {
...
class SecretFactory extends BaseFactory {
     ...
     */
    create(config) {
        return sealSecret(config.value, this[password]).then(sealed => {
            config.value = sealed;
    ...
    list(config) {
        return super
            .list(config)
            .then(secrets => Promise.all(secrets.map(secret => unsealSecret(secret, this[password]))));
    }
