...
async function convertStages(config) {
    const { pipeline, stages, groupEventId, jobs } = config;
    ...
    Object.entries(stages).forEach(([key, value]) => {
        ...
        const setupJobName = `${STAGE_SETUP_JOB_PREFIX}${key}`;
        const teardownJobName = `${STAGE_TEARDOWN_JOB_PREFIX}${key}`;

        if (Object.keys(jobs).find(j => j === setupJobName)) {
            newStage.setup = pipelineJobs.find(j => j.name === setupJobName).id;
        }
        if (Object.keys(jobs).find(j => j === teardownJobName)) {
...
async function createStages({ pipeline, groupEventId }) {
    ...
    const newStages = await convertStages({ pipeline, stages, groupEventId, jobs });
    const processed = [];
