...
class EventModel extends BaseModel {
    ...
    async getStageBuilds() {
        ...
        const stages = await stageFactory.list(listConfig);
        ...
        const stageBuildPromises = [];

        stages.forEach(stage => {
            stageBuildPromises.push(stageBuildFactory.get({ params: { stageId: stage.id, eventId: this.id } }));
        });

        const stageBuilds = await Promise.all(stageBuildPromises);
