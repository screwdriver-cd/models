...
async function convertStages(config) {
    const { pipeline, stages, jobs } = config;
    ...
    Object.entries(stages).forEach(([key, value]) => {
        ...
        const setupJobName = getFullStageName({ stageName: key, type: 'setup' });
        const teardownJobName = getFullStageName({ stageName: key, type: 'teardown' });
...
async function createStages({ pipeline, eventId }) {
    ...
    const convertedStages = await convertStages({ pipeline, stages, jobs });
    ...
    stagesToCreate.forEach(stage => {
        ...
        processed.push(createStageBuilds({ stageFactory, stageBuildFactory, stageConfig: { ...stage, eventId } }));
    });
