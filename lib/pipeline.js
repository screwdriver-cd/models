...
class PipelineModel extends BaseModel {
    ...
    getConfiguration(configuration) {
        ...
        return this.admin.then(user => user.unsealToken())
            ...
            .then(token => this.scmInfo.then((scmInfo) => {
                const config = {
                    scmUri: this.scmUri,
    ...
    /**
     * Fetch a pipeline's scmInfo
     * @property scmInfo
     * @return {Promise}
    */
    get scmInfo() {
        if (!this._scmInfo) {
            this._scmInfo = this.scm.lookupScmUri(this.scmUri);
        }
        ...
        Object.defineProperty(this, 'scmInfo', {
            enumerable: true,
            value: this._scmInfo
        });

        return this._scmInfo;
    }
