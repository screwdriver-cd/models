...
/**
 * Get Build timeout
 * Get a value of build-timeout from executor config
 * @method getBuildTimeout
 * @param  {Object}     executor executor
 * @return {Integer}             build time out (minutes)
 */
function getBuildTimeout(executorCfg) {
    // build timeout is set when executor is 'k8s' or 'k8s-vm'
    if (executorCfg.kubernetes && executorCfg.kubernetes.buildTimeout) {
        return executorCfg.kubernetes.buildTimeout;
    }
    ...
    start() {
        ...
        return this.job
            .then((job) => {
                ...
                const jwtExpiresSec = annotations[ANNOTATION_BUILD_TIMEOUT]
                    ? annotations[ANNOTATION_BUILD_TIMEOUT] * 60
                    : getBuildTimeout(this[executor]) * 60;

                return job.pipeline.then(pipeline =>
                    this[executor].start({
                        annotations,
                        apiUri: this[apiUri],
                        buildId: this.id,
                        ...
                        token: this[tokenGen](this.id, {
                            ...
                            jobId: job.id,
                            pipelineId: pipeline.id
                        }, pipeline.scmContext, jwtExpiresSec)
                    })
                        // update github
