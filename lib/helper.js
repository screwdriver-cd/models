...
function getExecutorName(annotations, pipeline, provider) {
    ...
    const executorAnnotationBeta = 'beta.screwdriver.cd/executor';
    const pipelineAnnotations = hoek.reach(pipeline, 'annotations', { default: {} });

    const executorFromAnnotations = annotations ? annotations[executorAnnotation] : annotations[executorAnnotationBeta];
    const executorFromPipelineAnnotations = pipelineAnnotations
        ? pipelineAnnotations[executorAnnotation]
        : pipelineAnnotations[executorAnnotationBeta];
