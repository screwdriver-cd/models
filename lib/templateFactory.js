        ...
        return this.list(templateListConfig).then(templates => {
            ...
            return Promise.all([
                ...
                templates.map(t => {
                    ...
                    const pipelineIds = pCount.filter(p => p.templateId === t.id).map(p => p.pipelineId);
                    const pipelinesUnique = new Set(pipelineIds);
                    // The number of pipelines that use this template,
                    // NOT the number of template uses in pipelines
                    const pipelineCount = pipelinesUnique.size;
