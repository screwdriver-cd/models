...
const {
    getAllRecords,
    getBuildClusterName,
    getAllActiveBuildClusters,
    getRandomCluster,
    getFullStageJobName,
    findBuildCluster,
    listDefaultBuildClusters
} = require('./helper');
...
class PipelineModel extends BaseModel {
    ...
    async sync(sha, chainPR = false) {
        ...
        // Fetch existing jobs now (before anything below mutates or persists),
        // since pipelineJobs is a self-memoizing getter and this same value is
        // reused later in this function instead of being re-queried.
        const existingJobs = await this.pipelineJobs;

        if (parsedConfig.errors) {
            const activeJobs = existingJobs.filter(j => !j.archived);

            // A pipeline that already has working jobs never legitimately parses to
            // the fallback config. Refuse to overwrite rather than mass-archive and
            // tear down crons on what may be a transient config-read failure
            // (e.g. a momentary SCM 404/auth blip) rather than a real user error.
            if (activeJobs.length > 0) {
                const message = `pipelineId:${this.id}: refusing to sync, config parse failed: ${parsedConfig.errors}`;
        ...
        if (!buildCluster) {
            ...
            const dbBuildClusterName = dbClusterAnnotations[buildClusterAnnotation];

            // This pipeline has build cluster setting
            if (dbBuildClusterName) {
                const defaultBuildClusters = await listDefaultBuildClusters();
                const currentBuildCluster = findBuildCluster(defaultBuildClusters, dbBuildClusterName, this.scmContext);

                // Check wether the current build cluster is one of the default build cluster
                if (currentBuildCluster) {
                    parsedConfig.annotations[buildClusterAnnotation] = dbBuildClusterName;
                } else {
                    // Reset default buildcluster
                    parsedConfig.annotations[buildClusterAnnotation] = getRandomCluster(
                        getAllActiveBuildClusters(defaultBuildClusters, this.scmContext, true)
                    );
                }
            }
        }
            ...
            await Promise.all(
                jobChunks.map(async job => {
                    ...
                    if (parsedConfigJobNames.includes(jobName)) {
                        ...
                        job.archived = true;
                        logger.info(`pipelineId:${pipelineId}: archiving job:${jobName}, not in parsed config`);
                        updatedJobs.push(await job.update());
                    }
    ...
    async getFirstRepoAdmin() {
        ...
        throw boom.forbidden('Pipeline has no repository admins');
    }
