...
/**
 * Sum daily event metrics
 * @method sumDailyEventMetrics
 * @param  {events}             events Array of events belong to the same date
 * @return {Object}                    Total queued time, image pull time, and duration of events
 */
const sumDailyEventMetrics = async (events) => {
    let totalQueuedTime = 0;
    let totalImagePullTime = 0;
    let totalDuration = 0;

    const promiseArray = events.map(async (e) => {
        const { queuedTime, imagePullTime, duration } = await eventMetrics(e);
...
class PipelineModel extends BaseModel {
    ...
    */
     ...
     * @param  {Boolean}  [config.aggregate]    Whether to aggregate the data
    ...
    async getMetrics(config = { startTime: null, endTime: null }) {
        ...
        if (!config.aggregate) {
            ...
            const promiseArray = events.map(async (e) => {
                ...
                const { duration, status, queuedTime, imagePullTime, builds }
                    = await eventMetrics(e);
        ...
        const allEvents = await getAllRecords.apply(this, ['getEvents', options, [[]]]);
        const aggregatedMetrics = [];
        const promiseArray = allEvents.map(async (sameDateEvents) => {
            const length = sameDateEvents.length;
            const { totalQueuedTime, totalImagePullTime, totalDuration }
                = await sumDailyEventMetrics(sameDateEvents);
