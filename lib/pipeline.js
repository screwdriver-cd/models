...
class PipelineModel extends BaseModel {
    ...
    async sync(sha, chainPR = false) {
        ...
        if (!buildCluster) {
            ...
            // This pipeline has buildcluster setting
            if (dbBuildClusterName) {
                const buildClusters = await buildClusterFactory.list();

                // Check whether the buildcluster is valid
                if (findBuildCluster(buildClusters, dbBuildClusterName, this.scmContext)) {
                    parsedConfig.annotations[buildClusterAnnotation] = dbBuildClusterName;
                } else {
                    // Use other valid buildcluster maneged by screwdriver to initialize the setting
                    parsedConfig.annotations[buildClusterAnnotation] = getRandomCluster(
                        getAllActiveBuildClusters(buildClusters, this.scmContext, true)
                    );
                }
            }
        }
