...
const TEMPLATE_NAME_REGEX_WITH_NAMESPACE =
    schema.config.regex.FULL_TEMPLATE_NAME_WITH_NAMESPACE;
...
class TemplateFactory extends BaseFactory {
    ...
    getFullNameAndVersion(fullTemplateName) {
        const [, templateName, versionOrTag] =
            TEMPLATE_NAME_REGEX.exec(fullTemplateName);
    ...
    create(config) {
        ...
        return super
            ...
            .then((latestTemplates) => {
                ...
                const configVersion = configMinor
            ...
            .then((matchTemplate) => {
                if (latestTemplate) {
                    const [, latestMajor, latestMinor] = VERSION_REGEX.exec(
            ...
            .then((targetTemplate) => {
                const newVersion = configMinor
                    ? `${configMajor}${configMinor}.0`
                    : `${configMajor}.0.0`;
                ...
                if (!targetTemplate) {
                    ...
                    const [, targetMajor, targetMinor, targetPatch] =
                        VERSION_REGEX.exec(targetTemplate.version);
    ...
    list(config) {
        if (config.params && config.params.name && !config.params.namespace) {
            ...
            return this._getNameAndNamespace(config.params.name).then(
                (parsedTemplateName) => {
    ...
    listWithMetrics(config) {
        ...
        return this.list(templateListConfig).then((templates) => {
            // templates: retreived list of all templates
            ...
            const metricPipelinesConfig = {
                params: { distinct: 'pipelineId', distinct: 'templateId' },
                readOnly: true,
            ...
            return Promise.all([
                ...
                jobFactory.list(metricPipelinesConfig),
            ]).then(([jCount, bCount, pCount]) =>
                templates.map((t) => {
                    const jobCount = jCount.find((j) => j.templateId === t.id);

                    const buildCount = bCount.find(
                        (b) => b.templateId === t.id,
                    );
                    const pipelineIds = pCount
                    ...
                    t.metrics = {
                        ...
                        pipelines: {
                            ids: pipelineIds,
                            count: pipelineIds ? pipelineIds.length : 0,
                        },
                    };
    ...
    get(config) {
        if (config.name && !config.namespace) {
            // eslint-disable-next-line no-underscore-dangle
            return this._getNameAndNamespace(config.name).then(
                (parsedTemplateName) => {
                    const { namespace, name } = parsedTemplateName;
    ...
    getTemplate(fullTemplateName) {
        const { templateName, versionOrTag, isExactVersion, isVersion } =
            this.getFullNameAndVersion(fullTemplateName);
