...
const { EXTERNAL_TRIGGER_ALL, CHECKOUT_URL, STAGE_SETUP_PATTERN } = require('screwdriver-data-schema').config.regex;
...
class PipelineModel extends BaseModel {
    ...
    async _createOrUpdateStages({ parsedConfig, pipelineId, stageFactory, prNum, pipelineJobs }) {
        ...
        const existingStageNames = prNum
            ? existingStages.map(stage => stage.name).filter(stageName => stageName.startsWith(`PR-${prNum}:`))
            : existingStages.map(stage => stage.name);
