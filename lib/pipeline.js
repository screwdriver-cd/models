...
const eventMetrics = async ({ event, downtimeJobs, downtimeStatuses }) => {
    ...
    let isDowntimeEvent =
        ((downtimeJobs.length === 0 || downtimeJobs.includes(jobId)) && downtimeStatuses.includes(status)) || false;
...
class PipelineModel extends BaseModel {
    ...
    remove() {
        ...
        const removeTokens = () => this.tokens.then(tokens => Promise.all(tokens.map(t => t.remove())));
