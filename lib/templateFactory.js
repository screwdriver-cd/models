...
class TemplateFactory extends BaseFactory {
        ...
        return this.list(templateListConfig).then(templates => {
            ...
            return Promise.all([
                ...
                templates.map(t => {
                    ...
                    const matchingPipelines = pCount.filter(p => p.templateId === t.id);
                    const pipelineIds = matchingPipelines.map(p => p.pipelineId);
                    const pipelineCount = matchingPipelines
                        .map(p => p.count)
                        .reduce((a, c) => a + c, 0);
    ...
    /**
     * Get a the latest template with metrics using the full template name
     * @method getWithMetrics
     ...
     */
    async getWithMetrics(fullTemplateName) {
        ...
        return jobFactory.list(metricPipelinesConfig).then(pipelines => {
            template.metrics = {
                // TODO: add rest of the metrics
                pipelines: {
                    // Sum values of count
                    count: pipelines.map(p => p.count).reduce((a, c) => a + c, 0),
                    pipelineIds: pipelines.map(p => p.pipelineId)
                }
            };
            return template;
        });
