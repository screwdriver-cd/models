...
const { STAGE_SETUP_PATTERN, PR_JOB_NAME, PR_STAGE_NAME } = schema.config.regex;
...
/**
 * List default build clusters
 * @return {Promise<Array>}
 */
async function listDefaultBuildClusters() {
    const buildClusters = await listBuildClustersByGroup();

    return buildClusters.default;
}

/**
 * check if pipeline org is authorized to use the build cluster
...
function getStageFromSetupJobName(jobName) {
    ...
    if (prJobName === null) {
        return stageSetupJobName[1];
    }

    return `${prJobName[1]}:${stageSetupJobName[1]}`;
}
...
function getStageName(workflowGraph, jobName) {
    ...
    if (currentNode && currentNode.stageName) {
        return prJobName ? `${prJobName[1]}:${currentNode.stageName}` : currentNode.stageName;
    }
