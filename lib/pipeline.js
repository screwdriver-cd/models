...
class PipelineModel extends BaseModel {
    ...
    */
    ...
    /**
     * Fetch all the active jobs that belong to this pipeline
     * Sorted by workflow order, followed by PR jobs
     * @return {Promise}    Resolves to an array of active jobs
     */
    getJobs(config) {
        const defaultConfig = {
            params: {
        ...
        .then((jobs) => {
            // If archived is true, return jobs
            ...
            // sort them by name. All PR jobs look like "PR-*"
            prJobs = prJobs.sort((job1, job2) =>
                parseInt(job1.name.slice(START_INDEX), 10) -
                parseInt(job2.name.slice(START_INDEX), 10)
            );
