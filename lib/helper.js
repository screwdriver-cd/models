 ...
 * @param  {String}       aggregate   Aggregate data by day/month/week/year
 ...
 */
async function getAllRecords(funcName, aggregate, opts, resultArray, date, index) {
    if (!['day', 'week', 'month', 'year'].includes(aggregate)) {
        throw new Error('Invalid aggregation option. ' +
            `${aggregate} is not one of these values ['day', 'week', 'month', 'year']`);
    }
    ...
    for (let i = 0; i < length; i += 1) {
        ...
        if (!dayjs(buildDate).isSame(dayjs(currentDate), aggregate)) {
            currentIndex += 1;
    ...
    return getAllRecords.apply(this,
        [funcName, aggregate, opts, resultArray, currentDate, currentIndex]);
}
