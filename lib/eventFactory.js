...
async function convertStages(config) {
    const { pipeline, stages, eventId, jobs } = config;
    ...
    Object.entries(stages).forEach(([key, value]) => {
        ...
        const setupJobName = `${STAGE_PREFIX}${key}:setup`;
        const teardownJobName = `${STAGE_PREFIX}${key}:teardown`;
 ...
 * @return {Promise}
 */
async function createStageBuilds({ stageFactory, stageBuildFactory, stageConfig }) {
    const stageCreateConfig = stageConfig;
    const stageBuildCreateConfig = {
        eventId: stageConfig.eventId,
        workflowGraph: stageConfig.workflowGraph,
        status: 'CREATED'
    };

    delete stageCreateConfig.eventId;
    delete stageCreateConfig.workflowGraph;

    const stage = await stageFactory.create(stageCreateConfig);

    stageBuildCreateConfig.stageId = stage.id;
    await stageBuildFactory.create(stageBuildCreateConfig);
 ...
 * @return {Promise}
 */
async function createStages({ pipeline, eventId }) {
    ...
    const convertedStages = await convertStages({ pipeline, stages, eventId, jobs, existingStages });
    ...
    stagesToUpdate.forEach(stage => {
        ...
        if (stage.startFrom) {
            updateStageConfig.startFrom = stage.startFrom;
        }
    ...
    // Create new stages and stage builds
    stagesToCreate.forEach(stage => {
        logger.info(`Creating stage:${JSON.stringify(stage)} for pipelineId:${pipelineId}.`);
        processed.push(createStageBuilds({ stageFactory, stageBuildFactory, stageConfig: stage }));
    });
                ...
                .then(pipeline =>
                    pipeline.token
                        ...
                        .then(event => {
                            ...
                            return createStages({ pipeline, eventId: event.id }).then(() => event);
                        })
                        .then(event => {
