...
const { SCM_STATE_MAP } = require('screwdriver-data-schema').config.scmCommons;
...
function getStatusConfig({ metadata }) {
    ...
    Object.keys(metadata).forEach(fieldName => {
        ...
        if (!['SUCCESS', 'FAILURE', 'PENDING'].includes(status)) {
            const message =
                `Skipping adding meta status for meta.status.${fieldName}: ` +
                'status is not valid. It must be one of SUCCESS, FAILURE, or PENDING';
    ...
    updateCommitStatus(pipeline) {
        return Promise.all([this.job, pipeline.token])
            .then(([job, token]) => {
                ...
                // Skiping handling the empty build status
