...
class TemplateFactory extends BaseFactory {
    ...
    getTemplate(fullTemplateName) {
        const { templateName, versionOrTag } = this.getFullNameAndVersion(fullTemplateName);

        return resolveVersionedModel({
            versionOrTag,
            getExactVersion: version => this.get({ name: templateName, version }),
            getTag: tag => {
                ...
                return templateTagFactory.get({ name: templateName, tag });
            },
            getLatestVersion: () =>
                this.list({
                    params: { name: templateName },
                    paginate: { page: 1, count: 1 }
                }).then(templates => templates[0] || null),
            listVersions: () => this.list({ params: { name: templateName } })
        });
    }
