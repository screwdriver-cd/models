...
function generateValue() {
    return nodeify.withContext(crypto, 'randomBytes', [TOKEN_LENGTH]).then(base64url);
}
...
function hashValue(value, salt) {
    return nodeify.withContext(crypto, 'pbkdf2', [value, salt, ITERATIONS, HASH_LENGTH, ALGORITHM]).then(base64url);
}
