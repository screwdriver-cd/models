...
class PipelineModel extends BaseModel {
    ...
    */
     ...
     * @param  {String}   [config.aggregate]    Whether to aggregate the data. For example, day/week/month/year
    ...
    async getMetrics(config = { startTime: null, endTime: null }) {
        ...
        if (!config.aggregate || config.aggregate === 'none') {
        ...
        const allEvents = await getAllRecords.apply(this,
            ['getEvents', config.aggregate, options, [[]]]);
